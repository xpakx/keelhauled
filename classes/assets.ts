...
export class Assets {
	...
	static async splitIndexedGridImage(
		img: HTMLImageElement,
		rows: number,
		cols: number,
		keys?: string[],
		...
		const cellWidth = img.width / cols;
		const cellHeight = img.height / rows;
		const tasks: Promise<[string, HTMLImageElement]>[] = [];
		...
		for (let y = 0; y < rows; y++) {
			for (let x = 0; x < cols; x++) {
				...
				const key = keys && keys.length > currKey ? keys[currKey++] :`${x},${y}`;
				const task = canvas.convertToBlob().then(async blob => {
					const url = URL.createObjectURL(blob!);
					const cellImg = await Assets.loadImage(url);
					URL.revokeObjectURL(url);
					return [key, cellImg] as [string, HTMLImageElement];
				});
				tasks.push(task);
			}
		}
